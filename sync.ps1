...
if ($Push) {
    ...
    # ── بوّابة السلاج ─────────────────────────────────────────────
    # تحويلُ كل اسم مستعار يُكتب في **جذر** الموقع بسلاج الاسم، مهما كان موضع
    # الملفّ (مقيسٌ لا مُفترَض). فإن ساوى سلاجُه سلاجَ صفحةٍ حلّ محلّها: 254
    # بايتًا مكان الفصل، أو تحويلٌ إلى نفسه لا ينتهي. ولا أثر لهذا في أوبسيديان
    # أصلًا، لأنّ روابطه بالأسماء لا بالمسارات. (CLAUDE.md 15/ح)
    Write-Host "→ فحص تصادم مسارات النشر..." -ForegroundColor Cyan
    # ‎--slug-roots self‎ لازم: هنا `content/` **هو** جذر الموقع، ومجلداته الداخلية
    # ليست جذور نشر. وبلا هذا التصريح تُقاس مداخل المعجم على مجلدها فتُعدّ أسماؤها
    # المستعارة حاجبةً لصفحاتها، وهي في الحقيقة اختصاراتٌ من الجذر إليها — 405
    # أخطاء كاذبة توقف نشرًا سليمًا. (CLAUDE.md 15/ح)
    python (Join-Path $Vault ".claude\skills\vault-audit\scripts\audit.py") `
        --root $Content --only slugs --glossary= --slug-roots self
    if ($LASTEXITCODE -ne 0) {
        throw "تصادم في مسارات النشر — اسمٌ مستعار يحجب صفحة، أو ملفّان بسلاج واحد"
    }
